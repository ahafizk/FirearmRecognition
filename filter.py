...
if __name__=='__main__':
    ...
    20 Hz when sampling frequency (fs) is 50 Hz to remove noise.
    Set conrner frequency or cutoff frequency  = 0.4*fs, order = 3
    these parameter can be used to filter accelerometer signal.

    import scipy.signal as sig
    sig.medfilt(signal) #default window is 3 and it must be odd, can be use 5

    so the total procedure:

    fsignal = sig.medfilt(signal,kernel_size = 5)
    lp = LowPassFilter()
